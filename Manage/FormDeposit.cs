        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (Convert.ToDouble(textBox1.Text) > 0)
            {
                if (label5.Text.Remove(0, 3).Equals("正常"))
                {
                    ...
                    note.CommandText = "insert into Action (no,time,id,money,type)values('" + no + "','" + System.DateTime.Now.ToString()+"','"+label2.Text.Remove(0,3)+"','"+textBox1.Text+"','1')";
                    ...
                    {
                        MessageBox.Show("记录无法保存,存款失败!", "提示");
                    }
                }
                else
                {
                    MessageBox.Show("账户状态不正常,存款失败!", "提示");
                }
            }
